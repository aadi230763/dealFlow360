  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">Discount tiers and approval chains</h1>
          <p className="text-sm text-ink-muted">
            When a quote mixes categories with different ceilings, the system computes a blended risk
            score and routes to the highest required level. All approvals, rejections and edits are
            logged with user, timestamp and reason.
          </p>
        </div>
      ...
      {/* Block 1: Tier Discount Ceilings */}
      <section className="rounded-sm border border-border bg-surface p-4">
        <h2 className="mb-3 text-sm font-semibold text-ink-muted">Tier Discount Ceilings</h2>
        <table className="w-full max-w-sm border-collapse text-sm">
          <tbody>
            {orderedTiers.map((tier) => (
              <tr key={tier.id} className="border-b border-border last:border-0">
                <td className="py-1.5 font-medium">{tier.name}</td>
                <td className="py-1.5 text-right">
                  <input
                    ...
                    className="w-20 rounded-sm border border-border bg-surface px-2 py-1 text-right tabular-nums"
                  />
      ...
      {/* Block 2: Category Discount Ceilings */}
      <section className="rounded-sm border border-border bg-surface p-4">
        <h2 className="mb-3 text-sm font-semibold text-ink-muted">Category Discount Ceilings</h2>
        <table className="w-full max-w-sm border-collapse text-sm">
          <tbody>
            {(categories ?? []).map((cat) => (
      ...
      {/* Block 3: Approval mapping */}
      <section className="rounded-sm border border-border bg-surface p-4">
        <h2 className="mb-3 text-sm font-semibold text-ink-muted">Approval mapping</h2>
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            <tr className="border-t border-border">
              <td className="py-1.5">Within tier or category limit</td>
              <td className="py-1.5 font-medium">No approval</td>
            ...
            {managerRule && (
              <tr className="border-t border-border">
                <td className="py-1.5">Over limit, blended risk medium</td>
                <td className="py-1.5 font-medium">Sales Manager</td>
            ...
            {financeRule && (
              <tr className="border-t border-border">
                <td className="py-1.5">Over limit, blended risk high</td>
                <td className="py-1.5 font-medium">Sales Manager then Finance</td>
                {(["min_blended", "min_peak", "min_erosion_amount"] as const).map((field) => (
                  <td key={field} className="py-1.5">
                    <input
                      type="number"
      ...
      {/* Bonus block: tier x category override matrix */}
      <section className="rounded-sm border border-border bg-surface p-4">
        <h2 className="mb-1 text-sm font-semibold text-ink-muted">Tier × Category ceiling matrix</h2>
        <p className="mb-3 text-xs text-ink-muted">
          A superset of the two blocks above — an explicit override per tier/category pair.
          Resolution order: this matrix, then the category default, then the tier base.
        </p>
